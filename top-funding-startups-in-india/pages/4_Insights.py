...
total_startups = df["StartupName"].nunique()
...
st.success(
f"Most funded startup: {top_startup.index[0]} "
...
if len(top_city) > 0:
st.info(
f"Highest funded city: {top_city.index[0]} "
...
st.success(
f"Top funded industry: {top_industry.index[0]} "
...
top_investor = df["InvestorsName"].value_counts()

```
if len(top_investor) > 0:
    st.info(
        f"Most active investor: {top_investor.index[0]} "
